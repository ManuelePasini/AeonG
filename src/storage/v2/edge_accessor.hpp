...
class EdgeAccessor final {
 ...
 public:
  ...
  //hjm begin
  EdgeAccessor(EdgeRef edge, EdgeTypeId edge_type, Vertex *from_vertex, Vertex *to_vertex, Transaction *transaction,
               Indices *indices, Constraints *constraints,storage::Gid from_gid,storage::Gid to_gid, Config::Items config, bool for_deleted = false)
      : edge_(edge),
        edge_type_(edge_type),
        from_vertex_(from_vertex),
        to_vertex_(to_vertex),
  ...
  //hjm begin
  storage::Gid fromGid()const noexcept{
    if(from_gid_)return *from_gid_;
    return from_vertex_->gid;
  }
  storage::Gid toGid()const noexcept{
    if(to_gid_)return *to_gid_;
    return to_vertex_->gid;
  }
  uint64_t transaction_st() const noexcept {return edge_.ptr->transaction_st;}//transaction_st;}
  // wzy edit 0512
  // uint64_t tt_te(){return edge_.ptr->tt_te;}
  uint64_t tt_te(){return (uint64_t)std::numeric_limits<int64_t>::max();}
  // wzy edit end
  Delta *getDeltas(){
  return edge_.ptr->delta;
  }
  //hjm end
 ...
 private:
  ...
  //hjm begin
  std::optional<storage::Gid> from_gid_;
  std::optional<storage::Gid> to_gid_;
  //hjm end
