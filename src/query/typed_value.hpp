...
class TypedValue {
 public:
  ...
  //hjm  begin
  explicit TypedValue(const storage::HistoryVertex &vertex, utils::MemoryResource *memory = utils::NewDeleteResource()) 
      : memory_(memory), type_(Type::HistoryVertex) {
    // delete vertex;
    new (&history_vertex_v) storage::HistoryVertex(std::move(vertex));
    // history_vertex_v=vertex;
    // std::cout<<"typed_value::415 "<<&history_vertex_v<<" "<<&vertex<<"\n";
  }

  explicit TypedValue(const storage::HistoryEdge &edge, utils::MemoryResource *memory = utils::NewDeleteResource()) 
      : memory_(memory), type_(Type::HistoryEdge) {
    // delete edge;
    new (&history_edge_v) storage::HistoryEdge(std::move(edge));
    // history_edge_v=edge;
    // std::cout<<"typed_value::421 "<<&history_edge_v<<" "<<&edge<<"\n";
  }
  //hjm end
  ...
  //hjm  begin
  explicit TypedValue(const storage::HistoryVertex &&vertex, utils::MemoryResource *memory = utils::NewDeleteResource()) noexcept
      : memory_(memory), type_(Type::HistoryVertex) {
    new (&history_vertex_v) storage::HistoryVertex(std::move(vertex));
    // history_vertex_v=vertex;
    // std::cout<<"typed_value::415 "<<&history_vertex_v<<" "<<&vertex<<"\n";
  ...
  explicit TypedValue(const storage::HistoryEdge &&edge, utils::MemoryResource *memory = utils::NewDeleteResource()) noexcept
      : memory_(memory), type_(Type::HistoryEdge) {
    new (&history_edge_v) storage::HistoryEdge(std::move(edge));
    // history_edge_v=edge;
    // std::cout<<"typed_value::421 "<<&history_edge_v<<" "<<&edge<<"\n";
  ...
  TypedValue &operator=(const storage::HistoryVertex &);
  //hjm end 
...
#define DECLARE_VALUE_AND_TYPE_GETTERS(type_param, field)          \
  ...
  DECLARE_VALUE_AND_TYPE_GETTERS(storage::HistoryVertex, HistoryVertex)
  //hjm end
