...
void AddMatching(const std::vector<Pattern *> &patterns, Where *where, Tt *tt,SymbolTable &symbol_table, AstStorage &storage,
  ...
  //hjm begin
  // std::cout<<"left here hjm: preprocess 131"<<"\n";
  if(tt){
    // matching.history_info_=std::make_pair(1,3);
    matching.history_infos_=std::make_pair(tt->tt_left_,tt->tt_right_);
    // auto *literal = dynamic_cast<PrimitiveLiteral *>(tt->tt_left_);
    // TypedValue value(literal->value_);
    // auto left = value.ValueInt();

    // auto *param_lookup = dynamic_cast<ParameterLookup *>(tt->tt_left_);
    // return TypedValue(parameters_.AtTokenPosition(param_lookup->token_position_));
    // auto left_literal=dynamic_cast<PrimitiveLiteral*>(tt->tt_left_)->value_;
    // auto right_literal=dynamic_cast<PrimitiveLiteral*>(tt->tt_right_)->value_;
    // matching.history_infos_=std::make_pair(left_literal,right_literal);
    // auto left=matching.history_infos_.first.ValueInt();
