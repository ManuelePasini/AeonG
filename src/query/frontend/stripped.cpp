...
StrippedQuery::StrippedQuery(const std::string &query) : original_(query) {
  ...
  // std::cout<<"stripped::48: "<<query<<std::endl;
  //std::cout<<original_<<std::endl;
  ...
  for (int i = 0; i < static_cast<int>(original_.size());) {
    ...
    //emplace_back在尾部添加元素，token和string中内容的对应，push_back的优化方法
  ...
  //所有的string、int、double、boolean都被替换成同样的在token_strings里，在literals_中保存了其对应的真实值
  ...
  //原query中的每一个token，记录在stripped query的索引
  ...
  for (int i = 0; i < static_cast<int>(tokens.size()); ++i) {
    ...
    //position_mapping有用信息位置对应token_string中的index
  ...
  while (it != tokens.end()) {
    ...
    if (it == tokens.end()) return;//wzy-没找到return
    // Skip RETURN;//找到就跳过
    ++it;
    ...
    while (it < tokens.end()) {
      ...
      auto jt = it;
      // We should track number of opened braces and parantheses so that we can //左大括号、圆括号
