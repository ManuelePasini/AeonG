...
namespace query {
using HistoryVertex=storage::HistoryVertex;
using HistoryEdge=storage::HistoryEdge;
...
TypedValue::TypedValue(const TypedValue &other, utils::MemoryResource *memory) : memory_(memory), type_(other.type_) {
  switch (other.type_) {
    ...
    case Type::Edge:
      ...
      return;
      //hjm begin
    case Type::HistoryVertex:
      new (&history_vertex_v) HistoryVertex(other.history_vertex_v);
      return;
    case Type::HistoryEdge:
      new (&history_edge_v) HistoryEdge(other.history_edge_v);
      return;
      //hjm end
    case Type::Path:
...
TypedValue::TypedValue(TypedValue &&other, utils::MemoryResource *memory) : memory_(memory), type_(other.type_) {
  switch (other.type_) {
    ...
    //hjm begin
    case Type::HistoryVertex:
      new (&history_vertex_v) HistoryVertex(std::move(other.history_vertex_v));
      break;
    case Type::HistoryEdge:
      new (&history_edge_v) HistoryEdge(std::move(other.history_edge_v));
      break;
      //hjm end
    case Type::Path:
...
//hjm begin
DEFINE_VALUE_AND_TYPE_GETTERS(HistoryVertex, HistoryVertex, history_vertex_v)
DEFINE_VALUE_AND_TYPE_GETTERS(HistoryEdge, HistoryEdge, history_edge_v)
//hjm end
...
std::ostream &operator<<(std::ostream &os, const TypedValue::Type &type) {
  switch (type) {
    ...
    case TypedValue::Type::HistoryVertex:
      return os << "historyvertex";
    case TypedValue::Type::HistoryEdge:
      return os << "historyedge";
      //hjm end
...
//hjm begin
DEFINE_TYPED_VALUE_COPY_ASSIGNMENT(const HistoryVertex &, HistoryVertex, history_vertex_v)
DEFINE_TYPED_VALUE_COPY_ASSIGNMENT(const HistoryEdge &, HistoryEdge, history_edge_v)
//hjm end
...
void TypedValue::DestroyValue() {
  switch (type_) {
    ...
    case Type::Edge:
      ...
      //hjm begin
    case Type::HistoryVertex:
      history_vertex_v.~HistoryVertex();
      break;
    case Type::HistoryEdge:
      history_edge_v.~HistoryEdge();
      break;
      //hjm end
    case Type::Path:
      path_v.~Path();
...
TypedValue operator==(const TypedValue &a, const TypedValue &b) {
  ...
  switch (a.type()) {
    ...
    case TypedValue::Type::Edge:
      ...
      //hjm begin
    case TypedValue::Type::HistoryVertex:
      return TypedValue(a.ValueHistoryVertex() == b.ValueHistoryVertex(), a.GetMemoryResource());
    case TypedValue::Type::HistoryEdge:
      return TypedValue(a.ValueHistoryEdge() == b.ValueHistoryEdge(), a.GetMemoryResource());
      //hjm end
    case TypedValue::Type::List: {
...
size_t TypedValue::Hash::operator()(const TypedValue &value) const {
  switch (value.type()) {
    ...
    case TypedValue::Type::Edge:
      ...
      //hjm begin
    case TypedValue::Type::HistoryVertex:
      return value.ValueHistoryVertex().gid.AsUint();
    case TypedValue::Type::HistoryEdge:
      return value.ValueHistoryEdge().gid.AsUint();
      //hjm end
    case TypedValue::Type::Path: {
