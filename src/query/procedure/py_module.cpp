...
//hjm begin
// PyObject *MakePyHistoryVertex(mgp_history_vertex &vertex, PyGraph *py_graph) {
//   MG_ASSERT(py_graph);
//   MG_ASSERT(py_graph->graph && py_graph->memory);

//   MgpUniquePtr<mgp_history_vertex> vertex_copy{nullptr, mgp_history_vertex_destroy};
//   if (RaiseExceptionFromErrorCode(CreateMgpObject(vertex_copy, mgp_history_vertex_copy, &vertex, py_graph->memory))) {
//     return nullptr;
//   }
//   auto *py_vertex = MakePyVertexWithoutCopy(*vertex_copy, py_graph);
//   if (py_vertex != nullptr) {
...
py::Object MgpValueToPyObject(const mgp_value &value, PyGraph *py_graph) {
  switch (value.type) {
    ...
    case MGP_VALUE_TYPE_HISTORY_VERTEX: {
      // py::Object py_mgp(PyImport_ImportModule("mgp"));
      // if (!py_mgp) return nullptr;
      // auto *v = value.history_vertex_v;
      // py::Object py_vertex(reinterpret_cast<PyObject *>(MakePyHistoryVertex(*v, py_graph)));
      Py_INCREF(Py_None);
      return py::Object(Py_None);
      // return py_mgp.CallMethod("Vertex", py_vertex);
    }
