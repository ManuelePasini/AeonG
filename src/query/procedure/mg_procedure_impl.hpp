...
struct mgp_value {
  ...
  //hjm begin
  mgp_value(mgp_history_vertex *, utils::MemoryResource *) noexcept;
  mgp_value(mgp_history_edge *, utils::MemoryResource *) noexcept;
  //hjm end
  ...
  union {
    ...
    mgp_edge *edge_v;
    //hjm begin
    mgp_history_vertex *history_vertex_v;
    mgp_history_edge *history_edge_v;
    //hjm end
    mgp_path *path_v;
    mgp_date *date_v;
...
//hjm begin
struct mgp_history_vertex {
  /// Allocator type so that STL containers are aware that we need one.
  /// We don't actually need this, but it simplifies the C API, because we store
  /// the allocator which was used to allocate `this`.
  using allocator_type = utils::Allocator<mgp_history_vertex>;

  // Hopefully VertexAccessor copy constructor remains noexcept, so that we can
  // have everything noexcept here.
  // static_assert(std::is_nothrow_copy_constructible_v<storage::HistoryVertex>);

  mgp_history_vertex(storage::HistoryVertex v, mgp_graph *graph, utils::MemoryResource *memory) noexcept
      : memory(memory), impl(v), graph(graph) {}

  mgp_history_vertex(const mgp_history_vertex &other, utils::MemoryResource *memory) noexcept
      : memory(memory), impl(other.impl), graph(other.graph) {}

  mgp_history_vertex(mgp_history_vertex &&other, utils::MemoryResource *memory) noexcept
      : memory(memory), impl(other.impl), graph(other.graph) {}

  mgp_history_vertex(mgp_history_vertex &&other) noexcept : memory(other.memory), impl(other.impl), graph(other.graph) {}

  /// Copy construction without utils::MemoryResource is not allowed.
  mgp_history_vertex(const mgp_history_vertex &) = delete;

  mgp_history_vertex &operator=(const mgp_history_vertex &) = delete;
  mgp_history_vertex &operator=(mgp_history_vertex &&) = delete;

  bool operator==(const mgp_history_vertex &other) const noexcept { return this->impl == other.impl; }
  bool operator!=(const mgp_history_vertex &other) const noexcept { return !(*this == other); };

  ~mgp_history_vertex() = default;

  utils::MemoryResource *GetMemoryResource() const noexcept { return memory; }

  utils::MemoryResource *memory;
  storage::HistoryVertex impl;
  mgp_graph *graph;
};
// hjm end
struct mgp_edge {
...
//hjm begin
struct mgp_history_edge {
  /// Allocator type so that STL containers are aware that we need one.
  /// We don't actually need this, but it simplifies the C API, because we store
  /// the allocator which was used to allocate `this`.
  using allocator_type = utils::Allocator<mgp_history_edge>;

  // Hopefully EdgeAccessor copy constructor remains noexcept, so that we can
  // have everything noexcept here.
  // static_assert(std::is_nothrow_copy_constructible_v<storage::HistoryEdge>);

  static mgp_history_edge *Copy(const mgp_history_edge &edge, mgp_memory &memory);

  mgp_history_edge(const storage::HistoryEdge &impl, mgp_graph *graph, utils::MemoryResource *memory) noexcept
      : memory(memory), impl(impl), from(impl.from_gid, graph, memory), to(impl.to_gid, graph, memory) {}

  mgp_history_edge(const mgp_history_edge &other, utils::MemoryResource *memory) noexcept
      : memory(memory), impl(other.impl), from(other.from, memory), to(other.to, memory) {}

  mgp_history_edge(mgp_history_edge &&other, utils::MemoryResource *memory) noexcept
      : memory(other.memory), impl(other.impl), from(std::move(other.from), memory), to(std::move(other.to), memory) {}

  mgp_history_edge(mgp_history_edge &&other) noexcept
      : memory(other.memory), impl(other.impl), from(std::move(other.from)), to(std::move(other.to)) {}

  /// Copy construction without utils::MemoryResource is not allowed.
  mgp_history_edge(const mgp_edge &) = delete;

  mgp_history_edge &operator=(const mgp_edge &) = delete;
  mgp_history_edge &operator=(mgp_edge &&) = delete;
  ~mgp_history_edge() = default;

  bool operator==(const mgp_history_edge &other) const noexcept { return this->impl == other.impl; }
  bool operator!=(const mgp_history_edge &other) const noexcept { return !(*this == other); };

  utils::MemoryResource *GetMemoryResource() const noexcept { return memory; }

  utils::MemoryResource *memory;
  storage::HistoryEdge impl;
  mgp_history_vertex from;
  mgp_history_vertex to;
};
// hjm end
