...
mgp_value_type FromTypedValueType(query::TypedValue::Type type) {
  switch (type) {
    ...
    case query::TypedValue::Type::HistoryEdge:
      return MGP_VALUE_TYPE_HISTORY_EDGE;//hjm end
...
query::TypedValue ToTypedValue(const mgp_value &val, utils::MemoryResource *memory) {
  switch (val.type) {
    ...
    case MGP_VALUE_TYPE_EDGE:
      ...
      //hjm begin
    case MGP_VALUE_TYPE_HISTORY_VERTEX:
      return query::TypedValue(val.history_vertex_v->impl, memory);
    case MGP_VALUE_TYPE_HISTORY_EDGE:
      return query::TypedValue(val.history_edge_v->impl, memory);
      // hjm end
...
mgp_value::mgp_value(const query::TypedValue &tv, mgp_graph *graph, utils::MemoryResource *m)
  ...
  switch (type) {
    ...
    //hjm begin
    case MGP_VALUE_TYPE_HISTORY_VERTEX: {
      utils::Allocator<mgp_history_vertex> allocator(m);
      history_vertex_v = allocator.new_object<mgp_history_vertex>(tv.ValueHistoryVertex(), graph);
      break;
    }
    case MGP_VALUE_TYPE_HISTORY_EDGE: {
      utils::Allocator<mgp_history_edge> allocator(m);
      history_edge_v = allocator.new_object<mgp_history_edge>(tv.ValueHistoryEdge(), graph);
      break;
    }
    //hjm end
    case MGP_VALUE_TYPE_PATH: {
...
mgp_value::mgp_value(const mgp_value &other, utils::MemoryResource *m) : type(other.type), memory(m) {
  switch (other.type) {
    ...
    case MGP_VALUE_TYPE_HISTORY_VERTEX: {
      utils::Allocator<mgp_history_vertex> allocator(m);
      history_vertex_v = allocator.new_object<mgp_history_vertex>(*other.history_vertex_v);
      break;
    }
    case MGP_VALUE_TYPE_HISTORY_EDGE: {
      utils::Allocator<mgp_history_edge> allocator(m);
      history_edge_v = allocator.new_object<mgp_history_edge>(*other.history_edge_v);
      break;
    }
    //hjm end
    case MGP_VALUE_TYPE_PATH: {
...
void DeleteValueMember(mgp_value *value) noexcept {
  ...
  switch (Call<mgp_value_type>(mgp_value_get_type, value)) {
    ...
    case MGP_VALUE_TYPE_EDGE:
      ...
      // hjm begin
    case MGP_VALUE_TYPE_HISTORY_VERTEX:
      allocator.delete_object(value->history_vertex_v);
      return;
    case MGP_VALUE_TYPE_HISTORY_EDGE:
      allocator.delete_object(value->history_edge_v);
      return;
      //hjm end
    case MGP_VALUE_TYPE_PATH:
...
mgp_value::mgp_value(mgp_value &&other, utils::MemoryResource *m) : type(other.type), memory(m) {
  switch (other.type) {
    ...
    case MGP_VALUE_TYPE_HISTORY_VERTEX: {
      // utils::Allocator<mgp_history_vertex> allocator(m);
      // history_vertex_v = allocator.new_object<mgp_history_vertex>(*other.history_vertex_v);
      // static_assert(std::is_pointer_v<decltype(hisvertex_v)>, "Expected to move vertex_v by copying pointers.");
      if (*other.GetMemoryResource() == *m) {
        history_vertex_v = other.history_vertex_v;
        other.type = MGP_VALUE_TYPE_NULL;
      } else {
        utils::Allocator<mgp_history_vertex> allocator(m);
        history_vertex_v = allocator.new_object<mgp_history_vertex>(std::move(*other.history_vertex_v));
      }
      break;
    }
    case MGP_VALUE_TYPE_HISTORY_EDGE: {
      ...
      // static_assert(std::is_pointer_v<decltype(edge_v)>, "Expected to move edge_v by copying pointers.");
      if (*other.GetMemoryResource() == *m) {
        history_edge_v = other.history_edge_v;
        other.type = MGP_VALUE_TYPE_NULL;
      } else {
        utils::Allocator<mgp_history_edge> allocator(m);
        history_edge_v = allocator.new_object<mgp_history_edge>(std::move(*other.history_edge_v));
      }
      break;
    }
    //hjm end
...
storage::PropertyValue ToPropertyValue(const mgp_value &value) {
  switch (value.type) {
    ...
    case MGP_VALUE_TYPE_EDGE:
      ...
      //hjm begin
    case MGP_VALUE_TYPE_HISTORY_VERTEX:
      throw ValueConversionException{"A historyvertex is not a valid property value! "};
    case MGP_VALUE_TYPE_HISTORY_EDGE:
      throw ValueConversionException{"An historyedge is not a valid property value!"};
      // hjm end
    case MGP_VALUE_TYPE_PATH:
...
//hjm begin
mgp_error mgp_history_vertex_copy(mgp_history_vertex *v, mgp_memory *memory, mgp_history_vertex **result) {
  return WrapExceptions([v, memory] { return NewRawMgpObject<mgp_history_vertex>(memory, *v); }, result);
}

void mgp_history_vertex_destroy(mgp_history_vertex *v) { DeleteRawMgpObject(v); }
//hjm end
...
std::ostream &PrintValue(const TypedValue &value, std::ostream *stream) {
  switch (value.type()) {
    ...
    case TypedValue::Type::HistoryEdge:
    //hjm end
