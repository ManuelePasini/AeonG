...
struct ExecutionContext {
  ...
  // wzy edit begin: add std::string addition
  ...
  std::map<uint64_t,std::vector<std::tuple<TypedValue,uint64_t,uint64_t>>> all_vertex;
  // std::map<uint64_t,std::vector<std::tuple<storage::HistoryEdge*,uint64_t,uint64_t>>> all_edge;
  // std::map<std::tuple<uint64_t,uint64_t,uint64_t>,storage::HistoryEdge*> all_edge;
  std::set<uint64_t> all_vertex_flag;
  std::string scan_op_name;
  Symbol input_symbol;
  uint64_t search_gid=0;
  storage::PropertyId search_property;
  storage::PropertyValue search_value;
  // std::set<uint64_t> all_edge_flag;
  // wzy edit end
};
