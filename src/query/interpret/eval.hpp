...
#undef UNARY_OPERATOR_VISITOR
  ...
  TypedValue Visit(PropertyLookup &property_lookup) override {
    ...
    switch (expression_result.type()) {
      ...
      case TypedValue::Type::HistoryVertex:
        return TypedValue(GetHistoryProperty(expression_result.ValueHistoryVertex(), property_lookup.property_), ctx_->memory);;//something need to change
      case TypedValue::Type::HistoryEdge:
        return TypedValue(GetHistoryProperty(expression_result.ValueHistoryEdge(), property_lookup.property_), ctx_->memory);
  ...
  TypedValue Visit(LabelsTest &labels_test) override {
    ...
    switch (expression_result.type()) {
      ...
      case TypedValue::Type::HistoryVertex: {
        const auto &vertex = expression_result.ValueHistoryVertex();
        for (const auto &label : labels_test.labels_) {
          // std::cout<<"label"<<label.AsUint()<<"\n";
          auto it = std::find(vertex.labels.begin(), vertex.labels.end(), GetLabel(label));
 ...
 private:
  ...
  storage::PropertyValue GetHistoryProperty(const TRecord &record_accessor, const std::string_view &name) {
    auto record_prop = record_accessor.properties;//.GetProperty(dba_->NameToProperty(name));
    auto prop_id=dba_->NameToProperty(name);
    if(record_prop.find(prop_id)==record_prop.end()){
      return storage::PropertyValue();
    }else{
      return record_prop[prop_id];
    }
    // return record_prop[prop_id];
  }
  template <typename TRecord>
  storage::PropertyValue GetHistoryProperty(const TRecord &record_accessor, PropertyIx prop) {
    auto record_prop = record_accessor.properties;//.GetProperty( ctx_->properties[prop.ix]);
    auto prop_id=ctx_->properties[prop.ix];
    if(record_prop.find(prop_id)==record_prop.end()){
      return storage::PropertyValue();
    }else{
      return record_prop[prop_id];
