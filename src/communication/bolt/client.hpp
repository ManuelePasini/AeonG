...
void WriteReportToFile(const QueryData &result, int queryId, std::string &query) {
    const std::string folderPath = "/home/AeonG/tests/results/query_results/output";
  ...
  QueryData Execute(const std::string &query, const std::map<std::string, Value> &parameters, int queryId = 0) {
